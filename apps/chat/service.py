...
CHAT_DISABLED_MESSAGE = "đoạn chat tạm thời không hoạt động"


def _blocked_chat_state(reason: str = "temporary_unavailable") -> dict:
	return {
		"can_send": False,
...
def _user_chat_problem(user: User | None) -> str:
	if not user:
		return "user_missing"
...
def get_direct_chat_state(user: User, target_user: User | None) -> dict:
	"""Return whether user can send messages to target_user in a direct chat."""
	problem = _user_chat_problem(target_user)
	if problem:
		return _blocked_chat_state(problem)

	if Block.objects.filter(
		Q(blocker=user, blocked=target_user) | Q(blocker=target_user, blocked=user)
	).exists():
		return _blocked_chat_state("blocked")

	return _active_chat_state()


def get_conversation_chat_state(user: User, conversation: Conversation) -> dict:
	"""Return chat state for the current user and the selected conversation only."""
	ensure_conversation_member(user, conversation)

	other_participants = list(conversation.participants.exclude(id=user.id))
	for participant in other_participants:
		state = get_direct_chat_state(user, participant)
		if not state.get("can_send", True):
			return {
				**state,
...
def validate_conversation_can_send_message(user: User, conversation: Conversation) -> None:
	state = get_conversation_chat_state(user, conversation)
	if not state.get("can_send", True):
		raise PermissionDenied(CHAT_DISABLED_MESSAGE)
...
def create_conversation(creator: User, participant_ids: Iterable[int]) -> Conversation:
	...
	participant_users = list(User.objects.filter(id__in=normalized_user_ids))
	existing_user_ids = {participant.id for participant in participant_users}
	if existing_user_ids != normalized_user_ids:
		raise ValidationError("One or more participants do not exist.")
	...
	for participant in participant_users:
		if participant.id == creator.id:
			continue
		state = get_direct_chat_state(creator, participant)
		if not state.get("can_send", True):
			raise PermissionDenied(CHAT_DISABLED_MESSAGE)
...
def list_conversations_for_user(user: User) -> list[dict]:
	...
	for conversation in conversations:
		...
		chat_state = get_conversation_chat_state(user, conversation)
...
def create_message(
	...
	*,
	...
	ensure_conversation_member(user, conversation)
	validate_conversation_can_send_message(user, conversation)
