...
from apps.chat.service import (
	...
	create_message,
	find_direct_conversation,
	get_conversation_chat_state,
	get_direct_chat_state,
	get_messages_for_conversation,
	get_or_create_direct_conversation,
	get_unread_count,
	list_conversations_for_user,
		...
		results.append(
			{
				...
				"avatar": _avatar_url(friend_user),
				"conversation_id": conversation_map.get(friend_user.id),
				"chat_state": get_direct_chat_state(user, friend_user),
			}
		)
		if limit and len(results) >= limit:
			break
...
def create_conversation_view(request):
	try:
		...
		return JsonResponse({"error": str(exc), "chat_state": {"can_send": False, "message": CHAT_DISABLED_MESSAGE}}, status=403)
	except ValidationError as exc:
		return JsonResponse({"error": _validation_error_message(exc)}, status=400)
		...
		return JsonResponse(
			{
				...
				"chat_state": get_conversation_chat_state(request.user, conversation),
			},
			status=200,
		)
	except PermissionDenied as exc:
	...
	return JsonResponse(
		{
			...
			"chat_state": get_direct_chat_state(request.user, friend_user),
		},
		status=200,
	)
...
def send_message_view(request, conversation_id):
	...
	try:
		...
		return JsonResponse(
			{
				"message": serialize_message(message, viewer=request.user),
				"chat_state": get_conversation_chat_state(request.user, conversation),
			},
			status=201,
		)
		...
		return JsonResponse(
			{
				"error": str(exc),
				"chat_state": {"can_send": False, "message": CHAT_DISABLED_MESSAGE},
			},
			status=403,
		)
	except ValidationError as exc:
...
def send_first_message_view(request, friend_id):
	...
	chat_state = get_direct_chat_state(request.user, friend_user)
	if not chat_state.get("can_send", True):
		return JsonResponse(
			{
				"error": CHAT_DISABLED_MESSAGE,
