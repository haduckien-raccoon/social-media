...
def sync_group_node(group) -> None:
    ...
    Neo4jFeedClient.execute(
        ...
        is_deleted=bool(getattr(group, "is_deleted", False)),
        owner_id=group.owner_id,
        owner_username=group.owner.username,
    )
...
def sync_group_member_edge(member) -> None:
    # Feed chỉ được dùng MEMBER_OF cho membership thật.
    # pending/rejected/banned/deleted-user phải xóa MEMBER_OF để không leak bài group.
    if (
        member.status != "approved"
        or getattr(member.user, "is_deleted", False)
        or not getattr(member.user, "is_active", True)
        or getattr(member.user, "is_banned", False)
    ):
        Neo4jFeedClient.execute(
            """
    ...
    SET u.username = $username,
        ...
        u.is_deleted = $user_is_deleted,
        u.is_banned = $user_is_banned
    ...
    Neo4jFeedClient.execute(
        ...
        user_id=member.user_id,
        username=member.user.username,
        user_is_active=bool(member.user.is_active),
        user_is_deleted=bool(getattr(member.user, "is_deleted", False)),
        user_is_banned=bool(member.user.is_banned),
        group_id=member.group_id,
        group_name=member.group.name,
        is_private=bool(member.group.is_private),
        is_activate=bool(member.group.is_activate),
        is_deleted=bool(getattr(member.group, "is_deleted", False)),
        role=member.role,
        joined_at=member.joined_at.isoformat(),
    )
...
def sync_group_post_relation(group_post) -> None:
    ...
    Neo4jFeedClient.execute(
        ...
        group_id=group_post.group_id,
        group_name=group_post.group.name,
        is_private=bool(group_post.group.is_private),
        is_activate=bool(group_post.group.is_activate),
        group_is_deleted=bool(getattr(group_post.group, "is_deleted", False)),
        post_id=group_post.post_id,
        status=group_post.status,
        is_deleted=group_post.is_deleted,
        is_pinned=group_post.is_pinned,
...
def get_recommended_feed_post_ids(
    ...
    *,
    ...
    CALL (me) {
        ...
        // This is a direct social source, not generic public discovery.
        MATCH (me)-[my_group_rel:MEMBER_OF]->(g:Group)<-[gp:IN_GROUP]-(post:Post)
        WHERE my_group_rel.status = "approved"
          AND coalesce(my_group_rel.is_deleted, false) = false
          AND gp.status = "approved"
          AND coalesce(gp.is_deleted, false) = false
          AND coalesce(me.is_deleted, false) = false
          AND coalesce(me.is_active, true) = true
          AND coalesce(me.is_banned, false) = false
    ...
    WHERE coalesce(post.is_deleted, false) = false
      ...
      AND (
        ...
        OR (post.privacy = "friends" AND (me)-[:FRIEND_WITH]-(author))
        OR EXISTS {
          MATCH (me)-[permission_rel:MEMBER_OF]->(permission_group:Group)<-[gp:IN_GROUP]-(post)
          WHERE permission_rel.status = "approved"
            AND coalesce(permission_rel.is_deleted, false) = false
            AND gp.status = "approved"
            AND coalesce(gp.is_deleted, false) = false
            AND coalesce(permission_group.is_deleted, false) = false
            AND coalesce(permission_group.is_activate, true) = true
        }
      )
    ...
    // - is_group_pinned: admin/owner pinned the post in that group
    OPTIONAL MATCH (post)-[post_group_rel:IN_GROUP]->(post_group:Group)<-[my_post_group_rel:MEMBER_OF]-(me)
    WHERE my_post_group_rel.status = "approved"
      AND coalesce(my_post_group_rel.is_deleted, false) = false
      AND post_group_rel.status = "approved"
      AND coalesce(post_group_rel.is_deleted, false) = false
      AND coalesce(post_group.is_deleted, false) = false
      AND coalesce(post_group.is_activate, true) = true
    WITH me, post, author, source_score, sources,
         count(DISTINCT post_group) AS shared_group_count,
         max(CASE WHEN coalesce(post_group_rel.is_pinned, false) THEN 1 ELSE 0 END) AS is_group_pinned
    ...
    OPTIONAL MATCH (author)-[author_group_rel:MEMBER_OF]->(author_group:Group)<-[my_author_group_rel:MEMBER_OF]-(me)
    WHERE author.id <> me.id
      AND author_group_rel.status = "approved"
      AND coalesce(author_group_rel.is_deleted, false) = false
      AND my_author_group_rel.status = "approved"
      AND coalesce(my_author_group_rel.is_deleted, false) = false
      AND coalesce(author_group.is_deleted, false) = false
      AND coalesce(author_group.is_activate, true) = true
    WITH me, post, author, source_score, sources,
         shared_group_count, is_group_pinned,
         count(DISTINCT author_group) AS common_group_with_author_count
    ...
    WHERE engaged_group_rel.status = "approved"
      AND coalesce(engaged_group_rel.is_deleted, false) = false
      AND group_member_rel.status = "approved"
      AND coalesce(group_member_rel.is_deleted, false) = false
      AND coalesce(group_member.is_deleted, false) = false
      AND coalesce(group_member.is_active, true) = true
      AND coalesce(group_member.is_banned, false) = false
      AND coalesce(engagement_group.is_deleted, false) = false
      AND coalesce(engagement_group.is_activate, true) = true
      AND EXISTS {
          MATCH (me)-[my_engagement_group_rel:MEMBER_OF]->(engagement_group)
          WHERE my_engagement_group_rel.status = "approved"
            AND coalesce(my_engagement_group_rel.is_deleted, false) = false
      }
      AND group_member.id <> me.id
      AND NOT (me)-[:BLOCKED]->(group_member)
      AND NOT (group_member)-[:BLOCKED]->(me)
